...
Personal academic website of **Xiaoqi Luo (罗骁齐)**, built with the
...
|------|--------|--------------|
| Home / About | `_pages/about.md` | Bio, profile photo (`assets/img/prof_pic.jpg`) |
| Publications | `_bibliography/papers.bib` | BibTeX entries |
| Projects | `_projects/*.md` | One file per project/internship |
| CV (rendered) | `_data/cv.yml` | Education, experience, skills, etc. |
| CV (PDF) | `assets/pdf/cv_xiaoqi_luo.pdf` | Replace with your latest PDF |
| News | `_news/*.md` | One file per announcement |
...
3. **LinkedIn / ORCID** — `_data/socials.yml`: fill `linkedin_username`,
   `orcid_id`.
4. **English proficiency** — `_data/cv.yml` `Languages`: adjust the level.
5. **Project teaser images** — `_projects/*.md` `img:` points to
   `assets/img/1..4.jpg` placeholders; replace with real screenshots/photos.
6. **Project videos** — each project page has a YouTube `VIDEO_ID` placeholder
   (and a commented `{% include video.liquid %}` for local files). Replace with
   real demo videos.
7. **CV PDF** — `assets/pdf/cv_xiaoqi_luo.pdf` is the current Chinese LaTeX CV.
   Consider adding an English version and pointing `cv_pdf` to it.
8. **Profile photo** — `assets/img/prof_pic.jpg` (currently your WeChat photo).
